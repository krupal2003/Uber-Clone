...
const ConfirmRide = (props) => {
    return (
        <div>
            <h5 onClick={() => { props.setConfirmRideOpen(false) }} className='absolute top-3 left-0 right-0 mx-auto w-8 text-center'><i className='ri-arrow-down-wide-fill w-8 text-3xl text-gray-400'></i></h5>
            <h2 className='text-xl font-semibold mb-3 mt-3 '>Confirm Your Ride</h2>
            <div className='flex flex-col items-center justify-between px-3 py-2'>
                ...
                <button onClick={() => {
                    props.setFoundingDriverPannel(true);
                    props.setConfirmRideOpen(false)
                    props.createRide();
                    }} className='w-full mt-4 px-3 py-2 bg-emerald-600 rounded-lg text-lg text-white font-semibold'>
                    Confirm Ride
                </button>
            </div>
        </div>
